  ...
  try {
    const transaction = await db.transaction(async (tx) => {
      const updated = await tx
        ...
        .where(
          and(
            eq(schema.users.userid, user.userid),
