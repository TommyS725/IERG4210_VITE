...
import { checkAuthToken, requireAdmin, requireSecure } from "../lib/middleware.js";
...
import { OrderDetail, orderDetailSchema } from "../model/orederDetail.js";
import { eq } from "drizzle-orm";
import { zValidator } from "@hono/zod-validator";

const merchant_email = "sb-fldzh30235098@business.example.com" as const;
const currency_code = "USD" as const;
...
function calculateTotalAmount(items: { price: number; quantity: number }[]) {
...
type unHashedDigest = z.infer<typeof unHashedDigestSchema>;

function createDigestHash(items: unHashedDigest["items"], salt: string) {
...
ordersHandler.post("/get-order-details", async (c) => {
  ...
  if (!parse.success) return BodyValidator.errorRespone(c);
  ...
  if (pids.length === 0) return c.text("Empty Cart", 400);
  ...
  const fullItems: z.infer<typeof fullItemSchema>[] = [];
  ...
  })) satisfies OrderDetail["purchase_units"][number]["items"];
  ...
  const hashedDigest = createDigestHash(
    fullItems.map((item) => ({
      pid: item.pid,
...
ordersHandler.post("/save-order", async (c) => {
  ...
  if (!UUID) return c.text("Invalid order", 400);
  const order = await db.query.orders.findFirst({
    where: (orders, { eq }) => eq(orders.UUID, UUID),
  });
  if (!order) return c.text("Invalid order", 400);

  const items = orderDetail.purchase_units[0]?.items;
  // console.log('items',items)
  if (!items || items.length === 0) return c.text("Invalid order detail", 400);
  ...
  const digestItems: unHashedDigest["items"] = [];
  for (const item of items) {
    const product = products.find((p) => p.name === item.name);
    if (!product) return c.text("Invalid order detail", 400);
    digestItems.push({
      pid: product.pid,
  ...
  if (hashedDigest !== order.digest) return c.text("Invalid order detail", 400);
...
ordersHandler.post("/cancel-order", async (c) => {
    ...
    if(order.orderDetails) return c.text("Order already processed",400);
    ...
    if(products.length!==items.length) return c.text("Invalid order detail",400);
...
const ordersToDisplay = 5 as const;
...
ordersHandler.get("/my-orders", async (c) => {
    ...
    try {
      const mutatedOrders = userOrders.map((order) => {
        if(!order.orderDetails) return order;
...
ordersHandler.get("/all-orders", requireAdmin,
zValidator("query", getAllOrdersQuerySchema,validationCallBack.query),
async (c) => {
