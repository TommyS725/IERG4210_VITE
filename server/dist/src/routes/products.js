...
products.get('/:pid', async (c) => {
    ...
    const data = await db.select().from(schema.products).where(eq(schema.products.pid, pid));
...
products.post('/', zValidator("form", postProductSchema, validationCallBack.form), async (c) => {
    const pid = ulid();
...
// update a product
products.put('/:pid', zValidator("form", putProductSchema, validationCallBack.form), async (c) => {
    const pid = c.req.param("pid");
    const fd = await c.req.formData();
    const image = fd.get("image");
    const { name, price: _price, description, inventory: _int, cid } = c.req.valid("form");
